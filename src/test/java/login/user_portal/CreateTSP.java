...
public class CreateTSP extends Login {
    public long[] ogrn = {6147413876241L,
            8045324322024L,
            3080943190688L,
            6077761483145L,
            8137370898976L,
    ...
    public void createTSPofUser() throws InterruptedException {
        ...
        driver.findElement(selectorUserTSPLogo).sendKeys(tspLogo);
        driver.findElement(xpathConfirmButton).click();
        driver.findElement(selectorUserTSPOGRN).sendKeys(String.valueOf(ogrn[ogrnrnd]));
    ...
    public void tspUserAddDiscount() throws InterruptedException {
        ...
        driver.findElement(selectorUserTSPOfferImage).sendKeys(imageDiscount);
        Thread.sleep(1000);
        driver.findElement(xpathConfirmButton).click();
        for (int i = 0; i < nmbAddressForDiscount; i ++) {
            driver.findElement(TSPOfferAddress(i)).sendKeys("Адрес предоставления № " + (i+1));
            Thread.sleep(2000);
            if (i == (nmbAddressForDiscount -1))
                break;
            else
                driver.findElement(xpathTSPUserAddress).click();
        }
